...
class OrderRepository extends Repository
{
    ...
    /**
     * Cancel order while payment failed.
     *
     * @param  \Webkul\Sales\Models\Order|int  $orderOrId
     * @return bool
     */
    public function cancelPayment($orderOrId)
    {
        /* order */
        $order = $this->resolveOrderInstance($orderOrId);
        ...
        Event::dispatch('sales.order.cancelPayment.before', $order);
        ...
        $this->updateOrderStatus($order, Order::STATUS_PAYMENT_FAILED);

        Event::dispatch('sales.order.cancelPayment.after', $order);
    ...
    public function updateOrderStatus($order, $orderState = null)
    {
        ...
        if($status == Order::STATUS_TO_PROCESS)
        {
           Event::dispatch('sales.order.payment.update-after', $order); 
        }
