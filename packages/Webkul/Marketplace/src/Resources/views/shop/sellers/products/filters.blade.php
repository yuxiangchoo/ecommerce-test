...
<div
    class="fixed bottom-0 z-50 grid w-full max-w-full grid-cols-[1fr_auto_1fr] items-center justify-items-center border-t bg-white px-5 ltr:left-0 rtl:right-0"
    v-if="isMobile"
>
    <!-- Filter Drawer -->
    <x-shop::drawer
    ...
    <script type="text/x-template" id="v-mp-filters-template">
        ...
        <template v-else>
            <div class="panel-side journal-scroll grid max-h-[1320px] min-w-[342px] grid-cols-[1fr] overflow-y-auto overflow-x-hidden max-xl:min-w-[270px] md:max-w-[400px] md:pr-7">
                ...
                <div class="flex h-[50px] items-center justify-between border-b pb-2.5 max-md:hidden">
                    <p class="text-lg font-semibold">
                        @lang('marketplace::app.shop.sellers.profile.products.filters.filters')
                    </p>
