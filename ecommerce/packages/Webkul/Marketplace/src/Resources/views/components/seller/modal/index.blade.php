...
>
    ...
    @isset($header)
        <template v-slot:header="{ toggle, isOpen }">
            <div {{ $header->attributes->merge(['class' => 'flex gap-5 justify-between items-center px-4 py-3 bg-white rounded-t-lg border-b']) }}>
                {{ $header }}
    ...
    @isset($footer)
        <template v-slot:footer>
            <div {{ $content->attributes->merge(['class' => 'px-4 py-2.5 bg-white rounded-b-lg border-t']) }}>
                {{ $footer }}
            </div>
        </template>
    @endisset
