    ...
    @foreach (app(\Webkul\Inventory\Repositories\InventorySourceRepository::class)->findWhere(['status' => 1]) as $inventorySource)
        ...
        <x-admin::form.control-group>
            <x-admin::form.control-group.label>
                {{ trans('admin::app.catalog.products.edit.inventories.quantity') }} ({{ $inventorySource->name }})
            </x-admin::form.control-group.label>
            ...
            <x-admin::form.control-group.control
                ...
                :rules="'numeric|min:0'"
                :value="$qty"
                :label="trans('admin::app.catalog.products.edit.inventories.quantity') . ' (' . $inventorySource->name . ')'"
            />
